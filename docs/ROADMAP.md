...
| Artwork is stored in **Postgres** in the deployment, not R2 | Cloudflare R2 requires a payment card. The catalogue's artwork is 4.1 MB against a 500 MB free tier, so a third `ObjectStorage` implementation was cheaper than a second account — ~90 lines, one migration, one branch. `STORAGE_BACKEND` still switches to R2 with no code change, which is the point of the seam. |
